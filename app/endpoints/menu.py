...
@app.post(BASE_API_URL + 'menus', response_model=schemas.MenuOut, status_code=status.HTTP_201_CREATED)
async def create_menu(menu: schemas.MenuIn, session: AsyncSession = Depends(get_async_session)) -> Menu:
    ...
    res = await session.execute(select(Menu).options(selectinload(Menu.submenus)).where(Menu.id == new_menu.id))
